...
public class DatabaseHandler extends Configs {
    ...
    public DataObject diffSum(DataObject msg) throws SQLException {
        ResultSet rs = null;
        ...
        double SK = 0;
        if (rs.next()) {
            SK += Integer.valueOf(rs.getString(Const.CAPITAL_AVL_SUM)) + Integer.valueOf(rs.getString(Const.CAPITAL_GRAT_INV));
        }
        ResultSet result = null;
        String borrowed = " SELECT * FROM " + Const.BORCAPITAL_TABLE + " INNER JOIN " + Const.COMPANY_TABLE + " ON " + Const.COMPANY_TABLE + "." +
                Const.COMPANY_ID + " = " + Const.BORCAPITAL_TABLE + "." + Const.BORCAPITAL_COMPANY_ID + " WHERE " + Const.BORCAPITAL_TABLE + "." + Const.COMPANY_ID +  " = ?";
        try {
        ...
        double res = 0;
        if (result.next()) {
            ZK += Double.valueOf(result.getString(Const.BORCAPITAL_WTH_PERCENT)) + (Double.valueOf(result.getString(Const.BORCAPITAL_CREDIT)) * (Double.valueOf(result.getString(Const.BORCAPITAL_PERCENT)) /100));
        }
        if (ZK > SK) {
            res = ZK - SK;
            msg.setPercent(" ЗК ");
            msg.setCredit(" СК ");
            msg.setMultiply(String.valueOf(res));
        }
        else {
            res = SK - ZK;
            msg.setPercent(" СК  ");
        ...
        msg.setAvailableSum(String.valueOf(percent1));
        msg.setGratInvest(String.valueOf(percent2));
        return msg;
    }
    ...
    public DataObject confirmRisk(DataObject msg) throws SQLException {
        ...
        msg.setCredit(String.valueOf(sum2));
        msg.setPercent(String.valueOf(sum));
    ...
    public DataObject diagCircle(DataObject msg) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                cap = Double.valueOf(rs.getString(Const.CAPITAL_AVL_SUM));
                invest = Double.valueOf(rs.getString(Const.CAPITAL_GRAT_INV));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        } catch (ClassNotFoundException e) {
        ...
        msg.setGratInvest(String.valueOf(invest));
        msg.setAvailableSum(String.valueOf(cap));
        return msg;
    }
